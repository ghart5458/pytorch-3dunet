...
def create_trainer(config: dict) -> 'UNetTrainer':
    ...
    # Remove tensorboard_formatter if present (no longer needed for MLflow)
...
class UNetTrainer:
    ...
    def __init__(self, model, optimizer, lr_scheduler, loss_criterion, eval_criterion, loaders, checkpoint_dir,
        ...
        # MLflow logging will be handled through the active run
        ...
        # No longer need tensorboard_formatter for MLflow logging
    ...
    def _log_lr(self):
        lr = self.optimizer.param_groups[0]['lr']
        mlflow.log_metric('learning_rate', lr, step=self.num_iterations)

    def _log_stats(self, phase: str, loss_avg: float, eval_score_avg: float):
        mlflow.log_metric(f'{phase}_loss_avg', loss_avg, step=self.num_iterations)
        mlflow.log_metric(f'{phase}_eval_score_avg', eval_score_avg, step=self.num_iterations)

    def _log_params(self):
        logger.info('Logging model parameters and gradients')
        # MLflow doesn't have histogram logging like TensorBoard
        # We'll skip parameter histograms for simplicity
        # Could implement statistics if needed (mean, std, min, max)
        pass


#Colors aren't right here. Messed with indents unsuccessfully.
    def _log_images(
            self, 
            input: np.ndarray, 
            target: np.ndarray, 
            ...
            prefix: str):
        # For simplicity, we'll skip image logging in MLflow
        # Image logging in MLflow requires saving files to disk first
        # which adds complexity. Focus on metrics for now.
        pass
